...
export const Menvane = async ({{ directory }}) => {{
  ...
  "chat.message": async (input, output) => {{
    ...
    if (context && Array.isArray(output.parts)) output.parts.push({{ id: `prt_${{crypto.randomUUID()}}`, sessionID: input.sessionID, messageID: output.message.id, type: "text", text: `\n\n${{context}}`, synthetic: true }})
  }},
...
mod tests {
    ...
    fn opencode_install_is_minimal_idempotent_and_preserving() {
        ...
        assert!(source.contains("id: `prt_${crypto.randomUUID()}`"));
