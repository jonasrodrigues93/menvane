...
pub const CONSOLIDATION_SYSTEM_PROMPT: &str = "Summarize the chronological session into intentions, actions, outcome, result, continuity, and candidate learnings. For every supplied handoff item, return exactly one operation that references that item; never reference the same item twice. If no handoff items are supplied, do not return keep, update, resolve, discard, replace, or uncertain operations. Every new continuity entry marked continues must omit item_id and have one corresponding handoff create operation. Only copy item_id from a supplied handoff item; never invent one. Never report live pending work only in the summary. Evaluate every delivered_context item exactly once. Mark it useful only with concrete session event evidence that it contributed to a decision, action, avoided repeated investigation, or successful result. Use unused when it was available but no use is observable, irrelevant when selection mismatched the current intent, incomplete when the delivered content lacked necessary detail, outdated or contradicted only with current evidence, harmful when it caused a wrong action, and unknown when evidence cannot support a stronger conclusion. Actively identify non-obvious reusable knowledge and cite exact supporting event identifiers from the packet. A memory may be supported by user statements, decisions, constraints, corrections, confirmed outcomes, or tool evidence; it never requires a tool event. Tool calls and results are strong signals for a possible playbook, but a new inferred playbook remains a candidate governed by its application lifecycle. Prefer memory for reusable project or environment facts, decisions, constraints, preferences, and gotchas. Prefer playbook for repeatable non-trivial procedures with validation and failure handling. Suppress open errors, pending work, temporary issue-tracker state, obvious facts, behavior evident from a canonical repository source, and claims without a plausible future retrieval scenario. Return zero knowledge operations only when no candidate passes these criteria. Return structured JSON only.";
